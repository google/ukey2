...
public class Ukey2Benchmark {
    ...
    public static class ConnectionState {
        D2DConnectionContext connContext;
        D2DConnectionContext serverConnContext;
        @Param({"10", "1024"})
        int sizeKibs;
        byte[] inputBytes;
        ...
        public void setup() throws Exception {
            Ukey2Handshake initiatorContext = Ukey2Handshake.forInitiator(HandshakeCipher.P256_SHA512);
            Ukey2Handshake serverContext = Ukey2Handshake.forResponder(HandshakeCipher.P256_SHA512);
            ...
            serverContext.parseHandshakeMessage(initiatorContext.getNextHandshakeMessage());
            serverContext.getVerificationString(10);
            initiatorContext.getVerificationString(10);
            serverContext.verifyHandshake();
            initiatorContext.verifyHandshake();
    ...
    public void encodeAndDecode(ConnectionState state) throws Exception {
        byte[] encoded = state.connContext.encodeMessageToPeer(state.inputBytes);
        state.serverConnContext.decodeMessageFromPeer(encoded);
    }
}
